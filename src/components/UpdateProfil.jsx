...
const UpdateProfil = () => {
    ...
    const error = useSelector((state) => state.errorReducer.userError); // Utilisation de userError
    const dispatch = useDispatch();
    const [followingPopup, setFollowingPopup] = useState(false);
    const [followersPopup, setFollowersPopup] = useState(false);

    // Utilisation d'un effet pour afficher des alertes en cas d'erreur
    useEffect(() => {
        if (error) {
            window.alert(error); // Afficher une alerte avec le message d'erreur
        }
    }, [error]);
